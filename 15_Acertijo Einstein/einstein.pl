...
altura(N) :- between(1, 5, N).  /*La funcion altura determina en que posicion puede estar cada una de las casas, en este caso desde la posicion 1 a la 5*/
...
% Casa %
alturaCasa(Altura, casa(Altura, _, _, _, _, _)). /*Esta función asigna las alturas correspondientes a cada casa*/
colorCasa(Color, casa(_, Color, _, _, _, _)). /*Esta función asigna los colores correspondientes a cada casa*/
nacionalidadCasa(Nacionalidad, casa(_, _, Nacionalidad, _, _, _)). /*Esta función asigna las nacionalidades correspondientes a cada casa**/
bebidaCasa(Bebida, casa(_, _, _, Bebida, _, _)). /*Esta función asigna las bebidas correspondientes a cada casa*/
cigarroCasa(Cigarro, casa(_, _, _, _, Cigarro, _)). /*Esta función asigna los cigarros correspondientes a cada casa*/
mascotaCasa(Mascota, casa(_, _, _, _, _, Mascota)). /*Esta función asigna las mascotas correspondientes a cada casa*/

sonVecinos(Casa1, Casa2) :-
        alturaCasa(N, Casa1),
        alturaCasa(M, Casa2),
        alturasVecinas(N, M). /*Regla que determina si dos casas son vecinas*/
    
    
estaALaIzq(Casa1, Casa2) :-
    alturaCasa(N, Casa1),
    alturaCasa(M, Casa2),
    izquierda(N, M). /*Regla que a partir de dos variables de altura determina que la altura de N es la altura de M-1*/

alturasVecinas(Altura1, Altura2) :- izquierda(Altura1, Altura2). /*Conjunto de reglas que determinan como es que las alturas de casas vecinas se encuentran posicionadas 
puesto que el que una casa que sea vecina de la otra puede significar que la casa1 esta a la izquierda de la casa2 o viceversa*/
alturasVecinas(Altura1, Altura2) :- izquierda(Altura2, Altura1).
0
izquierda(Altura1, Altura2) :- Altura1 is Altura2 - 1. /*Regla que resta uno a la altura2 para asignarselo a altura1*/
